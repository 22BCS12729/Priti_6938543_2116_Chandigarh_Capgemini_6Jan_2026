...
class Company : ICompany
{
    ...
    public (string categoryName, decimal totalValue) GetTopCategoryBySumOfProductPrices()
    {
        var result = categories
            .Select(c => new { Name = c.Name, Total = c.Products.Sum(p => p.Price) })
...
class Program
{
    static void Main()
    {
        ...
        {
            var a = Console.ReadLine().Split(" ");
            var c = categories.First(x => x.Id == Convert.ToInt32(a[0]));
            var pr = products.First(x => x.Id == Convert.ToInt32(a[1]));
            c.AddProduct(pr);
        }
        ...
        foreach (var item in company.GetProductsBelongsToMultipleCategory())
        {
