use database Subquery_Practise;
--AGENT Table
CREATE TABLE AGENTS 
(
...
----USING COMPARISON OPERATOR IN SINGLE ROW SUBQUERIES
--IN ORDERS TABLE
...
--SUBQUERIES OF HAVOING CLAUSE IN ORDERS TABLE
...
SELECT o.ord_num, o.agent_code, o.ord_date, o.ord_amount
FROM orders o
JOIN (
        SELECT agent_code, MIN(ord_amount) AS min_amount
        FROM orders
        GROUP BY agent_code
     ) m
ON o.agent_code = m.agent_code
AND o.ord_amount = m.min_amount;
